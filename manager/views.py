...
def show_today_applications(request, day, filter=None):
    if request.user.is_anonymous:
        return HttpResponseRedirect('/')
    ...
    if 'materials' in request.path:
        ...
        if filter == 'all':
        ...
        elif str(filter).isnumeric():
            id_foreman = int(filter)
            ...
            if str(_filter).isnumeric():
                id_foreman = int(_filter)
                _app = ApplicationMeterial.objects.filter(
                    app_for_day__construction_site__foreman_id=id_foreman,
                    status_checked=True,
                    app_for_day__date=current_day)
                out['filter'] = User.objects.get(id=id_foreman).last_name
            else:
    ...
    if filter == 'all':
        ...
        out['filter'] = User.objects.get(id=id_foreman).last_name
        if filter != _filter:
            set_var('filter_today_app', value=filter, user=request.user)
    else:
...
def send_all_applications(request, day):
    ...
    if is_admin(request.user):
        ...
        current_applications = ApplicationToday.objects.filter(
            status=ApplicationStatus.objects.get(status=STATUS_AP['approved']),
            date=current_day)
...
def get_conflicts_vehicles_list(current_day, c_in=0, all=False, lack=False, get_id=False, includeSave = False):
    ...
    app_list_today = ApplicationTechnic.objects.filter(
        ...
        # Q(technic_driver__driver__status=False) |
        Q(var_check=True))
...
def get_count_app_for_driver(current_day):
    ...
    _app = [_[0] for _ in ApplicationTechnic.objects.filter(
        ...
        Q(app_for_day__status=_status_send)).values_list('technic_driver_id')]
